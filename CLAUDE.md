...
- **API docs**: Swagger UI auto-generated by `@nestjs/swagger` at `http://localhost:3001/api` (dev)
...
│   ├── prisma/
│   │   ├── schema.prisma  # Single source of truth for DB schema (~457 lines, 12+ tables)
│   │   └── migrations/
│   └── test/              # E2E tests (jest-e2e.json)
├── packages/shared/src/index.ts  # Shared enums: UserRole, ArticleStatus, Platform,
│                                 #   ContentLanguage, PublishStatus, AgentType, TransactionType, etc.
│                                 # Shared interfaces: User, Story, Article, PlatformPublish, AutoPublishTask, etc.
...
- **Swagger/OpenAPI**: Controllers and DTOs are decorated with `@ApiTags`, `@ApiOperation`, `@ApiProperty` etc. Swagger UI is available at `/api` in dev. Keep decorators in sync with actual behavior.
...
- **Swagger UI**: Available at `http://localhost:3001/api` in dev. Useful for exploring endpoints and testing without a frontend.
