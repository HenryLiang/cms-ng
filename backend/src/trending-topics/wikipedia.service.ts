...
export class WikipediaService {
  ...
  async fetchOnThisDay(
    ...
    if (!cacheHit) {
      const raw = await this.callWikipediaApi(cfg.lang, cfg.variant, month, day);
      items = this.normalizeEvents(raw);
  ...
  /** 调用 Wikipedia On This Day REST API。按需挂代理。所有失败统一转 503。 */
  ...
  /**
   * 归一化 Wikipedia events 响应到通用条目形状（信息最大化保留）。
   * Wikipedia 结构：{ events: [{ text, year, pages: [{ normalizedtitle, extract, description, thumbnail, content_urls }] }] }
   *
   ...
   */
  private normalizeEvents(raw: any): any[] {
    ...
    return events.map((ev) => {
      ...
      // bestPage：优先有缩略图的（封面图），否则首个非年份页，最后回退 pages[0]
      const bestPage =
