...
- **UI i18n**: `next-intl` v4 with cookie-based locale mode, no URL routing. Supported UI locales: `zh-CN` (default) and `en`. Catalogs live in `frontend/messages/{zh-CN,en}/`.
- **Content language**: Content-level language support via `ContentLanguage` enum: `SIMPLIFIED_CHINESE`, `TRADITIONAL_CHINESE_HK`, `TRADITIONAL_CHINESE_CANTONESE`, `ENGLISH`. Defaults to `SIMPLIFIED_CHINESE`.
