...
export default function GEOPanel({ result, onClose, onApplySummary }: GEOPanelProps) {
  const t = useTranslations('panels.geo');
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-violet-200 bg-surface">
      <div className="flex items-center justify-between border-b border-violet-100 px-3 py-2">
        ...
        <button onClick={onClose} aria-label={t('close')} className="text-subtle transition-colors hover:text-foreground">
      ...
      <div className="max-h-96 space-y-4 overflow-auto p-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className={`flex flex-col items-center justify-center rounded-lg ${scoreBg(result.overallScore)} px-2 py-2`}>
            ...
            <span className="text-[10px] text-muted">{t('overallScore')}</span>
          </div>
          <div className={`flex flex-col items-center justify-center rounded-lg ${scoreBg(result.citationScore)} px-2 py-2`}>
            ...
            <span className="text-[10px] text-muted">{t('citationScore')}</span>
          </div>
          <div className={`flex flex-col items-center justify-center rounded-lg ${scoreBg(result.answerReadinessScore)} px-2 py-2`}>
            ...
            <span className="text-[10px] text-muted">{t('answerReadinessScore')}</span>
          </div>
        </div>
        ...
        {result.optimizedSummary && (
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <h4 className="text-xs font-medium text-muted">{t('citableSummary')}</h4>
              <button
                onClick={() => {
                  if (confirm(t('applySummaryConfirm'))) {
        ...
        {result.suggestedQuestions.length > 0 && (
          <div>
            <h4 className="mb-1.5 text-xs font-medium text-muted">{t('suggestedQuestions')}</h4>
            <div className="space-y-2">
              {result.suggestedQuestions.map((q, i) => (
                <div key={i} className="rounded-md border border-line p-2">
                  <p className="text-sm font-medium text-foreground">{t('questionLine', { question: q.question })}</p>
                  {q.answerSnippet && (
                    <p className="mt-1 text-xs leading-relaxed text-muted">{t('answerLine', { answer: q.answerSnippet })}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {result.keyStatements.length > 0 && (
          <div>
            <h4 className="mb-1.5 text-xs font-medium text-muted">{t('keyStatements')}</h4>
            <div className="space-y-2">
              {result.keyStatements.map((s, i) => (
                <div key={i} className="rounded-md border border-line p-2">
                  <p className="text-xs leading-relaxed text-foreground">&ldquo;{s.statement}&rdquo;</p>
                  {s.reason && (
                    <p className="mt-1 text-[11px] text-violet-600">{t('citationReason', { reason: s.reason })}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {result.entities.length > 0 && (
          <div>
            <h4 className="mb-1.5 text-xs font-medium text-muted">{t('coreEntities')}</h4>
        ...
        {result.suggestions.length > 0 && (
          <div>
            <h4 className="mb-1.5 text-xs font-medium text-muted">{t('suggestions')}</h4>
            <div className="space-y-2">
