          ...
          messages: [
            {
              role: 'system',
              content:
                '你是一位资深新闻编辑，擅长撰写吸引人的新闻标题。请用繁体中文回答。输出必须是有效的 JSON 格式。',
            },
            { role: 'user', content: prompt },
          ],
          ...
          messages: [
            {
              role: 'system',
              content:
                '你是一位资深新闻编辑，擅长提炼文章核心要点。请用繁体中文回答。',
            },
            { role: 'user', content: prompt },
          ],
    ...
    const messages = [
      {
        role: 'system',
        content:
          '你是一位资深新闻编辑和写作顾问，帮助记者改进稿件。请用繁体中文回答。回答要简洁、实用、有建设性。',
      },
      ...contextMessages,
      ...input.messages,
    ];
          ...
          messages: [
            {
              role: 'system',
              content:
                '你是一位资深新闻记者，擅长根据选题快速生成高质量的稿件初稿。请用繁体中文回答。输出必须是有效的 JSON 格式。',
            },
            { role: 'user', content: prompt },
          ],
          temperature: this.getTemperature(0.7),
...
- risk：风险提示`;
          ...
          messages: [
            {
              role: 'system',
              content:
                '你是一位资深新闻事实核查专家，擅长识别稿件中的事实性问题和风险。请用繁体中文回答。输出必须是有效的 JSON 格式。',
            },
            { role: 'user', content: prompt },
          ],
   ...
   */
  private async searchLatestNews(query: string): Promise<string> {
    const searchMessages = [
      {
        role: 'system',
        content:
          '你是一个新闻搜索助手。你的唯一任务是使用联网搜索工具查找最新新闻。请搜索用户提供的查询词，并返回搜索到的最新信息摘要。请用繁体中文回答。',
      },
      {
        role: 'user',
   ...
   */
  private async searchLatestNewsWithTavily(query: string): Promise<string> {
    const searchMessages = [
      {
        role: 'system',
        content:
          '你是一个新闻搜索助手。你的唯一任务是通过联网搜索工具查找最新新闻。请搜索用户提供的查询词，并返回搜索到的最新信息摘要。请用繁体中文回答。',
      },
      {
        role: 'user',
        content: `请搜索"${query}"的最新新闻和动态，返回最近6个月内的重要事件摘要。`,
    ...
    try {
      const searchSummary =
        this.searchProvider === 'tavily'
          ? await this.searchLatestNewsWithTavily(input.storyTitle)
          : await this.searchLatestNews(input.storyTitle);
      if (searchSummary) {
          ...
          messages: [
            {
              role: 'system',
              content:
                '你是一位资深新闻总编辑，擅长稿件质量评估和编辑指导。请用繁体中文回答。输出必须是有效的 JSON 格式。',
            },
            { role: 'user', content: prompt },
          ],
          temperature: this.getTemperature(0.4),
          ...
          messages: [
            {
              role: 'system',
              content:
                '你是一位资深新闻SEO专家，精通Google搜索算法和中文内容优化。请用繁体中文回答。输出必须是有效的 JSON 格式。',
            },
            { role: 'user', content: prompt },
          ],
          temperature: this.getTemperature(0.4),
          ...
          messages: [
            {
              role: 'system',
              content:
                '你是一位资深新闻编辑，擅长新闻写作和文字处理。请用繁体中文回答。直接输出处理后的文字，不要添加解释。',
            },
            { role: 'user', content: prompt },
          ],
