...
> **状态**: 已被后续实现替代
>
> 本文档描述的是 **2026-05-23 原始设计**,当时图片存储使用本地文件系统 (`./uploads/articles/{articleId}/`)。**截至 2026-06-04,存储层已迁移至腾讯云 COS 对象存储**,详情见:
...
[点击"生成选中配图"]
    ...
    │      └── 返回：临时图片 URL
    │
    ├──► [后端] 下载每张图片并上传到 COS 对象存储
    │      ├── 路径：cos.<region>.myqcloud.com/cms-ng/articles/{articleId}/{imageId}.png
...
[点击"删除"按钮]
    ...
    │      2. 从 COS 删除对象 cms-ng/articles/{id}/{imageId}.png
...
```json
{
  ...
  "images": [
    {
      "id": "img_abc123",
      "url": "https://cms-ng-prod-imgs-1300000000.cos.ap-shanghai.myqcloud.com/cms-ng/articles/article_001/img_abc123.png",
      "alt": "香港维多利亚港夜景封面图",
      "usageType": "COVER",
      "prompt": "香港维多利亚港夜景...",
      "seed": 42
...
**场景**：Seedream 返回了 URL，但上传到 COS 时网络中断或 COS 不可用。

**策略**：
1. 上传超时设置为 60 秒，超时后重试 2 次
2. 重试均失败后，标记该图为 failed，返回具体原因
3. 已上传成功的图片正常返回
4. 提示用户："第 N 张图片上传失败，其余图片已生成成功，失败的请重试"

### 8.3 存储空间不足

**场景**：COS bucket 满或配额不足（不适用于 COS，按量计费可忽略）。

**策略**：
1. COS 按量计费，理论上不会"满"
2. 若 COS 凭据无权写入，抛出 `ServiceUnavailableException`，前端提示"图片上传失败，请联系管理员"
3. 管理员检查 CAM 权限或 COS bucket 策略
...
**场景**：Seedream 返回的临时 URL 在上传到 COS 前已过期（有效期通常为 1 小时）。

**策略**：
1. 收到 URL 后立即下载并上传，不延迟
...
| 图片上传到 COS | ≤ 5 秒/张 | 从 Seedream URL 下载并上传到 COS |
| 面板首屏加载 | ≤ 1 秒 | 打开 AI 配图面板 |
| 图片预览加载 | ≤ 2 秒 | 从 COS 加载图片预览 |
