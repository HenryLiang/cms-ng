...
- **UI i18n**: `next-intl` v4 with cookie-based locale mode, no URL routing. Supported UI locales: `zh-CN` (default) and `en`. Catalogs live in `frontend/messages/{zh-CN,en}/`. Effective locale priority is user preference → system default → `zh-CN`.
- **Content language**: Content-level language support via `ContentLanguage` enum: `SIMPLIFIED_CHINESE`, `TRADITIONAL_CHINESE_HK`, `TRADITIONAL_CHINESE_CANTONESE`, `ENGLISH`. New content resolves user preference → system default → `SIMPLIFIED_CHINESE`; existing Story/Article language remains authoritative.
...
- Non-React library code: `libT()` in `frontend/src/i18n/client-dict.ts`.
- `LocaleSwitcher` writes `NEXT_LOCALE` cookie and calls `location.reload()`.
- Vitest globally mocks `next-intl` to return real `zh-CN` catalogs so existing Chinese assertions stay unchanged.
...
The repo is a Turbo monorepo with `frontend/`, `backend/`, `packages/shared/`, `scripts/`, `docker-compose.yml` (development middleware), and `docker-compose.prod.yml` (single-instance production application stack). See the filesystem for current structure.
...
Frontend conventions live in `frontend/CLAUDE.md` (which re-exports `frontend/AGENTS.md`).

### Major Subsystems

- **AI Layer** (`backend/src/ai/`): provider-agnostic `AIService` facade over `ChatCompletionProvider` implementations (deepseek/gemini/kimi/openai), plus a tool registry (`TavilySearchTool`).
- **Channels** (`backend/src/channels/`): platform adapters + WordPress publish service.
- **Auto-Publishing** (`backend/src/auto-publish/`): scheduled/triggered pipeline with kill-switch.
- **Billing & Payments** (`backend/src/billing/`): usage-based transactions, Alipay/WeChat Pay top-ups, balance alerts.
...
The topic-source extension point is `backend/src/trending-topics/sources/`: ordinary RSS/RSSHub feeds are declarative entries, while new mechanisms implement `TopicSourceAdapter` and register with `TopicSourceCatalog`. Example: `google-trends-realtime` renders Google Trends Trending Now (`/trending?geo=&hours=`) via Playwright headless browser — requires `PLAYWRIGHT_ENABLED=true` + a one-time `npx playwright install --with-deps chromium` on the host (production runs as a host process, not a container); fail-opens to the RSS daily `google-trends` source (`status:'degraded'`) when disabled or Chromium is missing. A second example: `newsnow/` vendors 30 Chinese hot-list/finance-flash sources (Baidu/Douyin/Toutiao/Cailianpress/Wallstreetcn/Jin10/HackerNews/GitHub Trending/...) from [ourongxing/newsnow](https://github.com/ourongxing/newsnow) (MIT) behind one `NewsnowTopicSourceAdapter`, plus 11 local tophub-mirror extensions (Xiaohongshu/WeChat hot articles & keywords/Weibo topics/Douban/Quark/Sogou/360/FT Chinese/Guancha/Chouti via RSSHub `/tophub/:id` routes, gated on the RSSHub container) - `NEWSNOW_ENABLED`/`NEWSNOW_SOURCES` gate it, `scripts/newsnow-smoke.ts` verifies per-source reachability. The `/dashboard/hot-topics` page (「实时热点」 menu, gated by `SystemFeature.HOT_TOPICS`) renders these sources as a newsnow-style card wall with 最热/实时 tabs driven by the definitions' `listType`. See `backend/CLAUDE.md` for the full contract and generic endpoints.
...
- **Elasticsearch 8.11 + IK** (containerized, optional): the other `docker-compose.yml` service (`docker/elasticsearch/Dockerfile` bakes the `analysis-ik` Chinese-analyzer plugin; bound to `127.0.0.1:9200`, single-node, `xpack.security` off — **never** re-bind to `0.0.0.0`: no auth + public = ransomware target). Powers media-library full-text/tag search and article title/content/tag search (`title^3 > tags^2 > content`). Off by default — enable with `ELASTICSEARCH_ENABLED=true` + `ELASTICSEARCH_NODE` in `backend/.env`; when disabled/unreachable/IK-missing, search fail-open degrades to MySQL `LIKE`. Media backfill/mapping evolution: `cd backend && npx ts-node scripts/reindex-media-search.ts`; article backfill: `cd backend && npm run search:reindex-articles` (both scripts support `--recreate --yes`). Start just ES with `docker compose up -d elasticsearch`; skip in dev via `dev-start.sh --no-es`.

`backend/.env` is the **single source of truth** for backend config in both dev and prod. Both `docker-compose.prod.yml` and the legacy `cms-ng-service.sh start --prod` host-process flow read it. Inside Docker, `localhost` refers to the container; use a reachable MySQL hostname or `host.docker.internal`. Template: `backend/.env.example`. Both files are gitignored except the `.example`.
...
- **Rate limiting**: NestJS Throttler is active by default. Set `NODE_ENV=test` to disable it globally (used for Playwright QA backend).
- **Production deploy (recommended)**: `docker-compose.prod.yml` runs one frontend, one backend, and an nginx proxy. `scripts/docker-prod.sh up` builds local images or pulls `CMS_NG_BACKEND_IMAGE`/`CMS_NG_FRONTEND_IMAGE`, then Compose runs a one-shot `migrate` service (`prisma migrate deploy`). Backend startup is gated on migration success; frontend/proxy startup is gated on health. The command applies committed migrations only—it never creates migrations, checks drift, or seeds data. On a fresh database, run `scripts/docker-prod.sh init-admin` after `up` for the idempotent SUPER_ADMIN bootstrap. Optional profiles: `rss` and `search`; Chromium and ffmpeg/ffprobe are opt-in with `INSTALL_PLAYWRIGHT=true` and `INSTALL_FFMPEG=true`. `CMS_NG_TRUST_PROXY_HOPS` defaults to one and may be set to two only when direct ingress around the outer proxy is blocked. A WeChat Pay key is mounted from `WECHAT_PAY_PRIVATE_KEY_FILE`, never baked into the image. `.github/workflows/container-images.yml` publishes amd64 backend/frontend images to GHCR on main.
- **Production deploy (legacy, systemd)**: `scripts/cms-ng-service.sh` runs production as systemd-managed host processes fronted by nginx (reverse proxy `:80`/`:443` -> `127.0.0.1:3000` frontend + `127.0.0.1:3001` backend). One-time setup (or after script upgrades): `./scripts/cms-ng-service.sh install --prod` writes `cms-ng-backend.service` / `cms-ng-frontend.service` (`Restart=on-failure` auto-restart + `enable` boot persistence) and `/etc/logrotate.d/cms-ng` (daily, 14 rotations). `start --prod` is the sole release entry point for this path: preflight -> build -> `systemctl stop` -> `prisma migrate deploy` -> `systemctl start` backend + frontend + RSSHub container -> health check -> admin init. `--no-build` skips build for config-only restarts. Prod logs via `logs --prod` or `journalctl -u cms-ng-backend -f`. nginx config at `/etc/nginx/conf.d/cms-ng.conf` routes to the backend by an explicit path-prefix list; **adding a backend `@Controller('xxx')` requires adding `xxx` to that list**, otherwise the route 404s in production. Use this path only for existing installations that have not moved to the Docker stack.
...
- **CI/CD**: GitHub Actions in `.github/workflows/`. `ci.yml` runs lint, Jest, Vitest, build, and backend e2e. `container-images.yml` publishes the two production images to GHCR. `deploy.yml` remains the manual legacy host-process deployment until its target server is migrated to Compose. Branch protection requires lint/test/build checks; Node is pinned by `.nvmrc` (24.18.0).
...
Functional regression lives in `tests/regression/` (`*.spec.ts`: smoke, auth/i18n, article workflow, AI, auto-publish, channels, cross-module, RBAC, …). `playwright.config.ts` does **not** auto-start servers - it assumes dev frontend `:3000` **and** a QA backend on `:3002` are already running. A `pageWithQA` fixture rewrites every `:3001` call to `:3002` so data flows to the separate `cms_ng_qa` DB, not dev. Run: `npx playwright test`; reports at `tests/regression/results/{html,run-summary.json,artifacts/}`. The QA backend is a manual `PORT=3002` NestJS process with `DATABASE_URL` -> `cms_ng_qa`. Seed QA test accounts with `cd backend && npx ts-node prisma/seed-qa-accounts.ts`. The `/regression-testing` skill has the full walkthrough.

## Scripts

Beyond `dev-start.sh` and `cms-ng-service.sh`:

- `scripts/docker-prod.sh` — validates and operates the production Docker Compose stack; `up` gates backend startup on the one-shot Prisma migration and `init-admin` bootstraps the SUPER_ADMIN account.
- `scripts/check-tiptap-dedup.sh` — checks for duplicate TipTap extensions in the frontend bundle.
- `scripts/cms-ng-service.sh` — service manager (install/start/stop/restart/status/logs); `--prod` runs the systemd-managed release (`install --prod` once to create the units; nginx + `node dist/src/main` + `next start` under systemd + rsshub container).
- `scripts/reindex-codebase-memory.sh` — checks whether the codebase-memory knowledge graph is stale and prints rebuild instructions.
...
- `PRD-super-admin-feature-switches.md`, `PRD-text-to-video.md` — system feature switches and video generation PRDs
