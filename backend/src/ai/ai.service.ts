...
export class AIService {
  ...
  constructor(
    ...
    this.searchProvider =
      this.config.get<string>('SEARCH_PROVIDER') || 'tavily';
   ...
   */
  private async checkAIBalance(
    userId: string,
    estimatedCost: number,
  ): Promise<void> {
    if (!this.billingService.isEnabled()) return;
    const sufficient = await this.billingService.checkBalance(
      userId,
      estimatedCost,
    );
    if (!sufficient) {
  ...
  private async deductLLMBilling(params: {
    ...
    if (!this.billingService.isEnabled()) return;
    try {
      const config = await this.billingService
   ...
   */
  private async getAuthorPersonaInstruction(
    authorSlug?: string,
  ): Promise<string> {
      ...
      fn: async () => {
        const response = await this.chatProvider.chatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是一位资深新闻编辑，擅长为记者发掘有价值的选题。${this.getLanguageInstruction(language)}。输出必须是有效的 JSON 对象格式，包含 suggestions 字段，不要包含任何其他文字。`,
            },
...
[
  { "title": "标题", "style": "风格标签", "reasoning": "推荐理由" }
    ...
    await this.checkAIBalance(userId, (2000 / 1000) * 0.02);
    const authorPersona = await this.getAuthorPersonaInstruction(
      input.authorSlug,
    );
      ...
      fn: async () => {
        const response = await this.chatProvider.chatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是一名有经验的新闻版面编辑，擅长起标题——不是 SEO 工具，是真正能抓人的标题。${this.getLanguageInstruction(language)}。${authorPersona}输出必须是有效的 JSON 格式。`,
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.8,
        ...
        return {
          result: headlines.slice(0, input.count ?? 5),
          tokensUsed: response.usage?.totalTokens,
        };
      },
      ...
      fn: async () => {
        const response = await this.chatProvider.chatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是一名新闻记者，擅长用一两句话把事件说清楚。${this.getLanguageInstruction(language)}。${authorPersona}`,
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.5,
...
正文前800字：${ctx.content.slice(0, 800)}`,
    ...
    const messages: ProviderChatMessage[] = [
      {
        role: 'system',
        content: `你是一名有经验的新闻编辑，帮同事改稿。说话直接、有用、不绕弯子，就像在编辑部茶水间聊天。${this.getLanguageInstruction(language)}。${authorPersona}
...
- risk：风险提示`;
      ...
      fn: async () => {
        const response = await this.chatProvider.chatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是一位资深新闻事实核查专家，擅长识别稿件中的事实性问题和风险。${this.getLanguageInstruction(language)}。输出必须是有效的 JSON 格式。`,
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.3,
      ...
      try {
        ...
        for (const result of searchResults) {
          ...
          if (data.type === 'standard' || data.type === 'disambiguation') {
            ...
            if (
              extract &&
              this.isWikipediaRelevant(query, pageTitle, extract)
            ) {
  ...
  private async performSearch(
    ...
    const searchMessages: ChatMessage[] = [
      {
        role: 'system',
        content: `你是一个新闻搜索助手。你的唯一任务是使用联网搜索工具查找最新新闻。请搜索用户提供的查询词，并返回搜索到的最新信息摘要。${this.getLanguageInstruction(language)}。`,
      },
      {
        role: 'user',
  ...
  async generateResearchKit(
    ...
    const [wikiResult, searchSummary] = await Promise.all([
      this.searchWikipedia(input.storyTitle).catch((err: unknown) => {
        const msg = err instanceof Error ? err.message : String(err);
        this.logger.warn(
    ...
    const wikipediaStatus: ResearchKitResult['wikipediaStatus'] =
      wikiResult.error
        ? 'api_error'
      ...
      fn: async () => {
        ...
        const result: ResearchKitResult = {
          timeline: Array.isArray(parsed.timeline)
            ? (parsed.timeline as ResearchKitTimelineEvent[])
            : [],
          people: Array.isArray(parsed.people)
            ? (parsed.people as ResearchKitPerson[])
            : [],
          data: Array.isArray(parsed.data)
            ? (parsed.data as ResearchKitDataPoint[])
            : [],
          opinions: Array.isArray(parsed.opinions)
            ? (parsed.opinions as ResearchKitOpinion[])
            : [],
      ...
      fn: async () => {
        const response = await this.chatProvider.chatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是一名有经验的新闻总编，每天要看几十篇稿子、给记者反馈。你的评估报告要像真实的编辑批注——直击要害、不客套。${this.getLanguageInstruction(language)}。输出必须是有效的 JSON 格式。`,
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.4,
      ...
      fn: async () => {
        const response = await this.chatProvider.chatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是一位新闻媒体的 SEO 运营专家，深知标题和关键词如何影响搜索流量。${this.getLanguageInstruction(language)}。输出必须是有效的 JSON 格式。`,
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.4,
      ...
      fn: async () => {
        const response = await this.chatProvider.chatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是一位新闻媒体的 GEO（生成式引擎优化）运营专家，深谙生成式 AI 如何抽取与引用网页内容。${this.getLanguageInstruction(language)}。输出必须是有效的 JSON 格式。`,
            },
            { role: 'user', content: prompt },
          ],
          temperature: 0.4,
...
5. 直接输出 prompt 文本，不要有任何解释或前缀`;
   ...
   */
  private static readonly SIZE_PIXEL_MAP: Record<
    string,
