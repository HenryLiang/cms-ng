...
- **Production deploy**: Production runs as systemd-managed host processes fronted by nginx (reverse proxy `:80`/`:443` → `127.0.0.1:3000` frontend + `127.0.0.1:3001` backend). One-time setup (or after script upgrades): `./scripts/cms-ng-service.sh install --prod` writes `cms-ng-backend.service` / `cms-ng-frontend.service` (`Restart=on-failure` auto-restart + `enable` boot persistence) and `/etc/logrotate.d/cms-ng` (daily, 14 rotations). `scripts/cms-ng-service.sh start --prod` is the **sole release entry point** and the standard SOP: `git pull` → `diff backend/.env.example backend/.env` → `./scripts/cms-ng-service.sh start --prod` → `status --prod` verify. The script runs: preflight (`backend/.env` present with `DATABASE_URL`/`JWT_SECRET`) → build (shared + `nest build` + `next build`) → `systemctl stop` → `prisma migrate deploy` → `systemctl start` backend (`node dist/src/main`) + frontend (`next start`) + RSSHub container → health check → admin init. `--no-build` skips build for config-only restarts (not for code/schema/dep changes). **Migrate runs BEFORE the app starts**, via `npx prisma migrate deploy` in the backend dir (NOT `migrate dev` — it only applies existing migrations, won't create new ones). Prod logs: `.cms-ng-backend.log` / `.cms-ng-frontend.log` (view via `logs --prod backend|frontend|rsshub`, or `journalctl -u cms-ng-backend -f`). nginx config at `/etc/nginx/conf.d/cms-ng.conf` — it routes to the backend by an explicit path-prefix list; **adding a backend `@Controller('xxx')` requires adding `xxx` to that list**, otherwise the route 404s in production. Frontend `/` returns 307 to `/dashboard`; `/login` returns 200. Backend `:3001/users` returns 401 without a JWT (healthy). nginx exposes `:80`/`:443` on `SERVER_IP`.
