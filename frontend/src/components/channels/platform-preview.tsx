        ...
        {publish.status === PublishStatus.GENERATING ? (
          ...
          <div className="space-y-2">
            {publish.adaptedTitle && (
              <p className="text-sm font-semibold text-foreground">{publish.adaptedTitle}</p>
            )}
            {publish.adaptedContent && (
              <div className="text-sm text-foreground leading-relaxed whitespace-pre-wrap max-h-48 overflow-auto">
                {publish.adaptedContent}
              </div>
            )}
