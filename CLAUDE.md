...
- **CI/CD**: GitHub Actions in `.github/workflows/`. `ci.yml` runs on PR/push to main — lint (`--max-warnings=0`), backend Jest, frontend Vitest, turbo build, backend e2e (MySQL+Redis services; e2e 初期 `continue-on-error`). `deploy.yml` = manual `workflow_dispatch` + `production` environment 审批 → SSH 部署 via `cms-ng-service.sh start --prod`. 分支保护要求 lint/test/build status checks; Node 由 `.nvmrc` 固定 (23.9.0)。
